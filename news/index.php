...
$sqlTopics = "SELECT * FROM `wn19_Topics`";
/*$sql = "
        ...
        <div class="table-responsive">
            <table class="table">
              ...
              {# process each row
    			...
    			$resultTopics = mysqli_query(IDB::conn(),$sqlTopics) or die(trigger_error(mysqli_error(IDB::conn()), E_USER_ERROR));

    			if(mysqli_num_rows($resultTopics) > 0){
      				while($roww = mysqli_fetch_assoc($resultTopics)){
        				if($row['CategoryID'] == $roww['CategoryID']){
          					echo '<li>';
    						echo '<a href="list.php?feedid='.$roww['CategoryID'].'">'.$roww['TopicName'].'</a>';
          					echo '</li>';
        				}
      				}      
    			}
    ...
    $resultTopics = mysqli_query(IDB::conn(),$sqlTopics) or die(trigger_error(mysqli_error(IDB::conn()), E_USER_ERROR));
    			...
    			if(mysqli_num_rows($resultTopics) > 0){
      				while($roww = mysqli_fetch_assoc($resultTopics)){
        				if($row['CategoryID'] == $roww['CategoryID']){
          					echo '<li>';
    						echo '<a href="topic_view.php?feedid='.$roww['TopicID'].'">'.$roww['TopicName'].'</a>';
          					echo '</li>';
        				}
      				}      
    			}
